...
// InlineEdit Component defined within this file
const InlineEdit = ({
  value,
  onChange,
  className = "",
...
export default function OrderCardGenerator() {
  ...
  // Local state for colors (editable per aggregated item)
  ...
  // Inline update for fabric content
  ...
  // Existing image upload function (for file input)
  ...
  // New function for drag-and-drop image uploads
const handleImageDrop = (itemId: string, e: React.DragEvent<HTMLElement>) => {
  e.preventDefault();
  ...
  return (
    <div className="max-w-[1200px] mx-auto p-5">
      ...
      {error && (
        <div className="p-2.5 text-red-600 bg-red-100 rounded mb-5">{error}</div>
      )}
            ...
            {aggregatedItems.map((item) => (
              <div key={item.id} className="flex flex-col">
                ...
                >
                  ...
                  {/* Image Section with Drag & Drop and Click-to-Select */}
                  <label
                    className="relative flex justify-center items-center my-2 h-40 mb-4 bg-gray-100 rounded cursor-pointer"
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={(e) => handleImageDrop(item.id, e)}
                  >
                    {images[item.id] ? (
                      <>
                        <img
                    ...
                    {/* Hidden File Input: Clicking the label triggers the file browser */}
                    <input
                      type="file"
                      accept="image/*"
